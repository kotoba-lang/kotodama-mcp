...
import { findTool, tools, type ClapDemoHandle } from "./tools.js";

export interface McpServerConfig {
  handle: ClapDemoHandle;
  name?: string;
  version?: string;
}

export function createYorishiroClapDemoMcpServer(config: McpServerConfig): Server {
  const server = new Server(
    { name: config.name ?? "etzhayyim-yorishiro-clap-demo", version: config.version ?? "0.1.0" },
    { capabilities: { tools: {} } },
  );
