...
import { findTool, tools, type DemoFixtureHandle } from "./tools.js";

export interface McpServerConfig {
  handle: DemoFixtureHandle;
  name?: string;
  version?: string;
}

export function createYorishiroDemoFixtureMcpServer(config: McpServerConfig): Server {
  const server = new Server(
    { name: config.name ?? "etzhayyim-yorishiro-demo-fixture", version: config.version ?? "0.1.0" },
    { capabilities: { tools: {} } },
  );
