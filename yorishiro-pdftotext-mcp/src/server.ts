...
import { findTool, tools, type PdftotextHandle } from "./tools.js";

export interface McpServerConfig {
  handle: PdftotextHandle;
  name?: string;
  version?: string;
}

export function createYorishiroPdftotextMcpServer(config: McpServerConfig): Server {
  const server = new Server(
    { name: config.name ?? "etzhayyim-yorishiro-pdftotext", version: config.version ?? "0.1.0" },
    { capabilities: { tools: {} } },
  );
