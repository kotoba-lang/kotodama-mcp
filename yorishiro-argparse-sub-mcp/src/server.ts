...
import { findTool, tools, type ArgparseSubHandle } from "./tools.js";

export interface McpServerConfig {
  handle: ArgparseSubHandle;
  name?: string;
  version?: string;
}

export function createYorishiroArgparseSubMcpServer(config: McpServerConfig): Server {
  const server = new Server(
    { name: config.name ?? "etzhayyim-yorishiro-argparse-sub", version: config.version ?? "0.1.0" },
    { capabilities: { tools: {} } },
  );
