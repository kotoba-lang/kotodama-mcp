...
import type { ArgparseDemoHandle } from "./tools.js";

export interface DefaultHandleOptions {
  binary?: string; // override the configured binary (otherwise uses "argparse-demo")
}
...
export function createDefaultArgparseDemoHandle(opts: DefaultHandleOptions): ArgparseDemoHandle {
  const binary = opts.binary ?? "argparse-demo";
  const handle: ArgparseDemoHandle = {
  async main(input) {
    const argv: string[] = [];
    ...
    {
      const v = (input.source_path ?? undefined);
