...
import { findTool, tools, type ArgparseDemoHandle } from "./tools.js";

export interface McpServerConfig {
  handle: ArgparseDemoHandle;
  name?: string;
  version?: string;
}

export function createYorishiroArgparseDemoMcpServer(config: McpServerConfig): Server {
  const server = new Server(
    { name: config.name ?? "etzhayyim-yorishiro-argparse-demo", version: config.version ?? "0.1.0" },
    { capabilities: { tools: {} } },
  );
