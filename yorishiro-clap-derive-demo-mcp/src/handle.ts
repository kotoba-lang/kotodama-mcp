...
import type { ClapDeriveDemoHandle } from "./tools.js";

export interface DefaultHandleOptions {
  binary?: string; // override the configured binary (otherwise uses "clap-derive-demo")
}
...
export function createDefaultClapDeriveDemoHandle(opts: DefaultHandleOptions): ClapDeriveDemoHandle {
  const binary = opts.binary ?? "clap-derive-demo";
  const handle: ClapDeriveDemoHandle = {
  async encode(input) {
    const argv: string[] = [];
