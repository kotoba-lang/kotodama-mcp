...
import { findTool, tools, type ClapDeriveDemoHandle } from "./tools.js";

export interface McpServerConfig {
  handle: ClapDeriveDemoHandle;
  name?: string;
  version?: string;
}

export function createYorishiroClapDeriveDemoMcpServer(config: McpServerConfig): Server {
  const server = new Server(
    { name: config.name ?? "etzhayyim-yorishiro-clap-derive-demo", version: config.version ?? "0.1.0" },
    { capabilities: { tools: {} } },
  );
