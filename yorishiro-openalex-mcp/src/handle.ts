...
export function createDefaultOpenalexHandle(opts: DefaultHandleOptions): OpenalexHandle {
  ...
  async search_works(input) {
    ...
    const queryKeys: ReadonlySet<string> = new Set<string>(["search","filter","sort","per_page","page"]);
    ...
    const init: RequestInit = {
      method: "GET",
      headers: {
        "User-Agent": "etzhayyim-yorishiro-openalex-mcp/0.1",
  ...
  async search_authors(input) {
    const inp = input as Record<string, unknown>;
    const pathKeys: ReadonlySet<string> = new Set<string>([]);
    const queryKeys: ReadonlySet<string> = new Set<string>(["search","filter","per_page","page"]);
    const bodyKeys: ReadonlySet<string> = new Set<string>([]);
    let path = "/authors";
    for (const key of pathKeys) {
      const token = `{${key}}`;
