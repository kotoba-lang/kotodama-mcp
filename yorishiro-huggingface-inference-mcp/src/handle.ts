...
export function createDefaultHuggingfaceInferenceHandle(opts: DefaultHandleOptions): HuggingfaceInferenceHandle {
  ...
  async extract_features(input) {
    ...
    const pathKeys: ReadonlySet<string> = new Set<string>(["model_id"]);
    const queryKeys: ReadonlySet<string> = new Set<string>([]);
    const bodyKeys: ReadonlySet<string> = new Set<string>(["inputs","wait_for_model"]);
    ...
    const init: RequestInit = {
      method: "POST",
      headers: {
        "User-Agent": "etzhayyim-yorishiro-huggingface-inference-mcp/0.1",
