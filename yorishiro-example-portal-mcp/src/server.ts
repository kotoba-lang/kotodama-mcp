...
import { findTool, tools, type ExamplePortalHandle } from "./tools.js";

export interface McpServerConfig {
  handle: ExamplePortalHandle;
  name?: string;
  version?: string;
}

export function createYorishiroExamplePortalMcpServer(config: McpServerConfig): Server {
  const server = new Server(
    { name: config.name ?? "etzhayyim-yorishiro-example-portal", version: config.version ?? "0.1.0" },
    { capabilities: { tools: {} } },
  );
